...
function setupInputScreen() {
  ...
  nextBtn.position(width/2 + 250, height/2 + 200);
...
function draw() {
  ...
  if (finalMode) drawFinalStage && drawFinalStage();
}
...
function stageBalls() {
  ...
  for (let i = 0; i < messages.length; i++) {
    balls.push(new Ball(random(100, width - 100), random(100, height - 100), 30, messages[i]));
  }
}
...
function mousePressed() {
  ...
  } else zoomedB = null; // back to normal
}

function createFinalButton() {
  if (newPersonBtn) newPersonBtn.remove();

  newPersonBtn = createButton("New Messages");
...
class Ball {
  ...
  drawLineTo(otherBalls) {
    ...
    for (let other of otherBalls) {
      ...
      if (d < 90) { stroke(255, map(d, 0, 90, 255, 0)); 

...
let sound;
let messages = [];
let checkFigures = false; // NEW flag for figure animation
...
let notes = [
  ...
  698.46,783.99,880.0,987.77,1046.5,1174.66,
...
function draw() {
  ...
  if (!ballMode && !finalMode) drawTypingGalaxy();
  if (ballMode) drawBallsStage();
  if (finalMode) drawFinalStage && drawFinalStage(); // avoid error if undefined
}
...
function enterGalaxy() {
  ...
  check = true;
  checkFigures = true; // start figure animation immediately
  inputBox.hide();
  submitBtn.hide();
  nextBtn.show();
...
class Star {
  constructor() {
    ...
    this.isStar = random() < 0.5;
  }

  update() {
    let speed1 = checkFigures ? 5 : 1;
    let growth1 = checkFigures ? 3 : 1;
  ...
  if (checkFigures) {
    ...
    if (this.isStar) {
      ...
      beginShape();
      vertex(0, -this.size);
      vertex(this.size * 0.38, -this.size * 0.38);
      vertex(this.size, -this.size * 0.2);
...
class Ball {
  constructor(x, y, rad, message) {
    this.x = x; this.y = y; this.rad = rad; this.message = message;
    this.xSpd = random(-3, 3); this.ySpd = random(-3, 3);
    this.hue = random(0, 360); this.s = 100; this.b = 100;
    this.t = random(150, 255); this.aspeed = random(0.5, 1);
  }

  move() {
    this.x += this.xSpd; this.y += this.ySpd;
    this.t += this.aspeed;
  ...
  drawLineTo(otherBalls) {
    ...
    for (let other of otherBalls) {
      ...
      if (d < 90) { stroke(255, map(d, 0, 90, 255, 0)); line(this.x, this.y, other.x, other.y); }
    }
  }
  ...
  display(zoomed = false) {
    ...
    noStroke();
    if (zoomed) {
      ellipse(width/2, height/2, this.rad*30, this.rad*5);
      fill(255); textAlign(CENTER, CENTER); textSize(32); text(this.message, width/2, height/2);
    } else {
  ...
  isMouseOver() { return dist(mouseX, mouseY, this.x, this.y) < this.rad + 5; }
}

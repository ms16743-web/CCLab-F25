...
let messages = [];   // <--- STORED MESSAGES HERE (PERMANENT)
...
// ------------------------------------------------------------
// STAGE 1 — Moving balls that contain messages
// ------------------------------------------------------------
function stageBalls() {
  ballMode = true;
  finalMode = false;
...
// ------------------------------------------------------------
// BUTTON → go to FINAL STAGE
// ------------------------------------------------------------
function createFinalButton() {
  if (newPersonBtn) newPersonBtn.remove();

  newPersonBtn = createButton("See All Messages");
  newPersonBtn.position(50, 50);
...
function drawFinalStage() {
  ...
  if (!newPersonBtn) {
    ...
    newPersonBtn.position(width - 200, height - 100);
    newPersonBtn.mousePressed(() => {
      newPersonBtn.remove();
      setupInputScreen();
    });
